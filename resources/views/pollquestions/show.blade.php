...
@section('content')
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            if (select.attr("type") == "checkbox") {
                var selectedValues=[]; 
                $('.question_3 input:checked').each(function() {
    ...
    <form method="post" name="question" action="{{action('PollResponseController@store')}}" accept-charset="UTF-8">
        ...
        @while(isset($question))
            ...
            <div class="question question_{{$question->id}}" data-id="{{$question->id}}" {{empty($question->display_criteria)?"":'style=display:none data-display-criteria='.$question->display_criteria}}>
                ...
                @elseif($question->type == "select")
                    @if ($question->max_alternatives < 2)
                        @foreach($question->alternatives_array as $alternative)
                            <div class="radio">
                                <label><input type="radio" data-original-name="response[{{$question->id}}]" name="response[{{$question->id}}]" value="{{$alternative}}" onclick="document.question.submit.disabled=false;">{{$alternative}}</label>
                            </div>
                        @endforeach
                    @else
